...
class SLOValidator:
    ...
    def __init__(self, max_agents: int = 10):
        ...
        self.orchestrator = Orchestrator(self.config, world_id="slo_test")
    ...
    async def validate_observation_delivery_latency(
        ...
        # Generate events and measure observation delivery latency
        for i in range(num_samples):
            ...
            try:
                # Trigger event that should generate observation
                ...
                # Get observation (this includes the delivery latency)
                observation = await self.facade.observe(agent_id)
    ...
    async def validate_intent_throughput(
        ...
        async def submit_intents_for_agent(agent_id: str) -> None:
            ...
            while time.perf_counter() - agent_start_time < duration_seconds:
                intent: Intent = {
                    "kind": "SLOTest",
                    "payload": {
                ...
                try:
                    effect, observation = await self.facade.step(agent_id, intent)
        ...
        async def agent_operation_sequence(
            ...
            for i in range(operation_count):
                ...
                if i % 3 == 0:
                    ...
                    await self.facade.step(agent_id, intent)
                elif i % 3 == 1:
                    # Observation
                    await self.facade.observe(agent_id)
                else:
                    # Event broadcast
                    await self.orchestrator.broadcast_event(
                        {
    ...
    async def run_all_slo_validations(self) -> list[SLOResult]:
        ...
        slo_tests = [
            ...
            ("Cancellation Latency", self.validate_cancellation_latency, (100.0, 100)),
            ("Intent Throughput", self.validate_intent_throughput, (100.0, 8.0)),
            ("Non-blocking Operations", self.validate_non_blocking_operations, (300,)),
            ("Memory Stability", self.validate_memory_stability, (20.0, 30.0)),
        ]
...
class TestSLOValidation:
    ...
    async def test_observation_delivery_latency_slo(self):
        ...
        try:
            ...
            assert result.passed, f"Observation delivery latency SLO failed: {result.actual_value:.2f}ms > {result.target_value}ms"
    ...
    async def test_cancellation_latency_slo(self):
        ...
        try:
            ...
            assert result.passed, f"Cancellation latency SLO failed: {result.actual_value:.2f}ms > {result.target_value}ms"
            assert (
    ...
    async def test_intent_throughput_slo(self):
        """Test intent processing throughput SLO (≥100 intents/sec per agent)."""
        validator = SLOValidator(max_agents=5)

        try:
            await validator.setup()
            result = await validator.validate_intent_throughput(100.0, 5.0)

            assert result.passed, f"Intent throughput SLO failed: {result.actual_value:.2f} < {result.target_value} intents/sec/agent"
            assert (
                result.actual_value >= 100.0
            ), "Should achieve ≥100 intents/sec per agent"
            assert (
                result.details["success_rate"] >= 0.95
    ...
    async def test_non_blocking_operations_slo(self):
        ...
        try:
            ...
            assert result.passed, f"Non-blocking operations SLO failed: blocking detected or P99 latency {result.actual_value:.2f}ms too high"
    ...
    async def test_memory_stability_slo(self):
        ...
        try:
            ...
            assert result.passed, f"Memory stability SLO failed: growth {result.actual_value:.2f}MB > {result.target_value}MB"
            assert result.actual_value <= 30.0, "Memory growth should be limited"
            assert (
                result.details["operations_completed"] > 0
